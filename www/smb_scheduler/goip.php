...
*/
		...
		if($ErrMsg!="")
			...
			WriteSuccessMsg("<br><li>删除goip设备成功</li>","goip.php");
...
*/
		...
		sendto_xchanged($send);
		WriteSuccessMsg("<br><li>向goip($_GET[name])发送指令完毕</li>","goip.php");
	}
	else if($action=="reboot_module"){
		$send[]=pack('LCL', $checksum, MODULE_REBOOT, $_GET['name']);
		sendto_xchanged($send);
		WriteSuccessMsg("<br><li>向goip的模块($_GET[name])发送指令完毕</li>","goip.php");
	}
	else $action="main";
...
if($action=="main")
{
	...
	while($row=$db->fetch_array($query)) {
		...
		if($row['dev_disable'] == 0){
			...
			$row['disable'] = '<font color="#FF0000">已禁用</font>';
		}
