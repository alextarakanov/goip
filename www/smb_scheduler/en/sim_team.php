...
if(isset($_GET['action'])) {
	...
	if($action=="recv"){
		...
		//echo $rcount;
		$fenye=showpage2("sim_team.php?action=recv&id=$id&",$page,$count,$perpage,true,true,"rows","myform","boxs");
		//if(_GET)
		$query=$db->query("(select sim.*,sim_team_name, 1 as 'in' from sim left join sim_team using (sim_team_id) where sim.sim_team_id=$id order by sim_name  limit 99999999)
				union 
				(select *,NULL,0 from sim where sim_team_id=0 order by sim_name  limit 99999999) 
				union
				(select sim.*,sim_team_name,2 from sim left join sim_team using (sim_team_id) where sim.sim_team_id!=0 and sim.sim_team_id!=$id order by sim_name  limit 99999999) 
...
elseif($action=="receivers"){
	...
	WriteSuccessMsg("<br><li>Successfully Modified</li>","sim_team.php?action=recv&id=$id");
                ...
                //echo $rcount;
                $fenye=showpage2("sim_team.php?action=grecv&id=$id&",$page,$count,$perpage,true,true,"rows","myform","boxs");
                //if(_GET)
                $query=$db->query("(select device_line.*,sim_team_name, 1 as 'in' from device_line left join sim_team on sim_team.sim_team_id=device_line.goip_team_id where device_line.goip_team_id=$id order by line_name ASC  limit 99999999)
                                union 
                                (select *,NULL,0 from device_line where goip_team_id=0 order by line_name ASC limit 99999999) 
                                union
                                (select device_line.*,sim_team_name,2 from device_line left join sim_team on sim_team.sim_team_id=device_line.goip_team_id where device_line.goip_team_id!=0 and device_line.goip_team_id!=$id order by line_name ASC limit 99999999)
                                        LIMIT $start_limit,$perpage");
...
elseif($action=="greceivers"){
	...
	if($send)
                sendto_xchanged($send);
        WriteSuccessMsg("<br><li>Successfully Modified</li>","sim_team.php?action=grecv&id=$id");
...
elseif($action=="del")
{
	...
	if($ErrMsg!="")
		...
		WriteSuccessMsg("<br><li>Successfully Delete</li>","sim_team.php");
...
elseif($action=="saveadd")
{
	...
	if($ErrMsg!="")
		...
		if(empty($rs[0])){
			...
			sendto_xchanged($send);
			WriteSuccessMsg("<br><li>Add group success</li>","sim_team.php");
		}
		else{
...
/*
		...
		$send[]=my_pack2(SCH_UPDATE, $Id, $_POST[work_time]*60, $_POST[sleep_time]);
		sendto_xchanged($send);
		//if(!$crs[0]) //	改变了群	
...
if($action=="main")	
{
	...
	$fenye=showpage("sim_team.php?",$page,$count,$perpage,true,true,"row(s)");
