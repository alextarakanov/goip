...
$select2.="</select>";
$select2.="线路:<select name=\"line_name\"  style=\"width:90px\" >\n\t<option value=\"0\" $ch>All</option>\n";
...
$select="搜索项目<select name=\"column\"  style=\"width:120px\" >";
$select.="\t<option value='number' $c_selected[number]>通话号码</option>\n";
$select.="\t<option value='duration' $c_selected[duration]>通话时长</option>\n";
...
$select.="</select>\n";
$select.="搜索类型<select name=\"type\"  style=\"width:95px\" >";
...
$select.="</select>\n";
$select.="搜索关键字<input type=\"text\" name=\"s_key\" value='$_REQUEST[s_key]' size=16>&nbsp;\n";
$select.="<input type=\"submit\" name=\"submit_value\" value=\"搜索\">\n";
...
if($_REQUEST['submit_value']=='导出'){
        ...
        $fenye=showpage("?$pages&action=$action&column=$column&type=$type&s_key=$_REQUEST[s_key]&order_key=$_REQUEST[order_key]&order=$_REQUES
T[order]&start_time=$start_time&end_time=$end_time&",$page,$count,$perpage,true,true,"编");
...
<tr><td>
<?php echo $select ?> <input type="submit" name="submit_value" value="导出">
</td></tr>
</form>
</table>
