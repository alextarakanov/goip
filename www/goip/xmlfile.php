...
require_once("global.php");
	...
	function do_cron($db,$crontime,$port,$count)
	{
		...
		if($flag)
			WriteSuccessMsg("已加成功入到计划中，共{$count}条信息.", 'xmlfile.php');
		else 
			WriteErrMsg("已加成功入到计划中,但goipcron进程未响应，请检查该进程");		
	}
...
if($_POST['action']=='send'){
	...
	*/
	...
	$sql="INSERT INTO message (msg,userid,crontime,stoptime,type,tel,prov,uid,msgid) VALUES ";
	for($i=0;$i<sizeof($xml["sms_msg"]);$i++){
		...
		$sql.="('$msg',$_SESSION[goip_userid],$crontime,$stoptime,3,'$tel','$prov','$uid','$id')";	
	}
	if($i>0){
		$db->query($sql);
...
//print_r($vals);
	if( $vals[0]['tag']=='UID' || $vals[0]['tag']=='uid') $uid=$vals[0]['value'];
	//echo $uid;
	//$data = implode("",file($_FILES['xmlfile']['tmp_name'])) or die("could not open XML input file"); 
	//$obj = new xml($data,"xml");
	//$uid=addslashes($_POST['uid']);
