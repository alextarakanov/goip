...
if($_REQUEST['submit_value']=='Export'){
        ...
        $fenye=showpage("?$pages&action=$action&column=$column&type=$type&s_key=$s_key&order_key=$_REQUEST[order_key]&order=$_REQUES
T[order]&start_time=$start_time&end_time=$end_time&",$page,$count,$perpage,true,true,"rows");
	...
	$t_info="全部";
	if($_REQUEST[line_name])  {
		$where="where line_name='$_REQUEST[line_name]'";
		$pages="line_name=$_REQUEST[line_name]";
		$t_info="GoIP线路ID:$_REQUEST[line_name]";
	}
	else if($_REQUEST[sim_name])  {
		$where="where sim_name='$_REQUEST[sim_name]'";
		$pages="sim_name=$_REQUEST[sim_name]";
		$t_info="SIM Slot ID:$_REQUEST[sim_name]";
	}
	$query=$db->query("SELECT count(*) AS count FROM call_record $where ");
	$row=$db->fetch_array($query);
	$count=$row['count'];
...
<td wIdth="70" height="30"><strong>导航:</strong></td>
    <td height="30"><a href="?line_name=<?php echo $_REQUEST[line_name] ?>&sim_name=<?php echo $_REQUEST[sim_name] ?>" target=main>刷新</a>&nbsp;|&nbsp;<a href="call_record.php" target=main>所有通话记录</a></td>
  </tr>
</table>
