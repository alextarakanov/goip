...
if(isset($_GET['action'])) {
	...
	if($action=="recv"){
		...
		//if(_GET)
		$query=$db->query("(select goip.*,goip_team_name, 1 as 'in' from goip left join goip_team using (goip_team_id) where goip.goip_team_id=$id order by line_name ASC limit 999999)
				union 
				(select *,NULL,0 from goip where goip_team_id=0 order by line_name ASC limit 999999) 
				union
				(select goip.*,goip_team_name,2 from goip left join goip_team using (goip_team_id) where goip.goip_team_id!=0 and goip.goip_team_id!=$id order by line_name ASC limit 999999)
