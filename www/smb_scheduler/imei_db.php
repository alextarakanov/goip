...
*/  
	...
	elseif($action=="savemodify")
	{
		...
		if($ErrMsg!="")
			...
			$db->query("UPDATE imei_db SET imei='$imei',used='$used',imsi='$imsi' WHERE id='$Id'");
...
*/
				...
				if($ext=='.csv')
				{
					...
					while($data   =   fgetcsv($fp,'1024',',')){   
						$row++;
						//echo "data[0] <br>";
						$db->query("insert into imei_db set imei='".$data[0]."', imsi='".$data[1]."' on duplicate key update `imei`='".$data[0]."',imsi='".$data[1]."'");
					}
					fclose($fp); 
				...
				else if($ext=='.xls') 
				{
					...
					for ($row=0;$row<count($return[Sheet1]);$row++)
					{
						...
						$db->query("insert into imei_db set imei='".$return[Sheet1][$row][0]."', imsi='".$return[Sheet1][$row][1]."' on duplicate key update `imei`='".$return[Sheet1][$row][0]."',imsi='".$return[Sheet1][$row][1]."'");
...
if($action=="main")
{
	...
	$query=$db->query("SELECT * from imei_db ORDER BY used,imei LIMIT $start_limit,$perpage");
	while($row=$db->fetch_array($query)) {
		$rsdb[]=$row;
	}
}
