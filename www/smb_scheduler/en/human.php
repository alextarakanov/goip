...
elseif($action=="saveadd")
{
	...
	foreach($a as $limit){ //each period
		...
		$i++;
                ...
                //echo $rcount;
                $fenye=showpage2("human.php?action=grecv&id=$id&",$page,$count,$perpage,true,true,"rows","myform","boxs");
                //if(_GET)
...
elseif($action=="greceivers"){
        ...
        if($delstrs){
                ...
                //$db->query("update device_line set next_auto_dial_time='0' where id in ($delstrs)");
