...
//cal = new Calendar(by, ey,1,strFormat); //初始化英文版，0 爲中文版 
cal = (cal==null) ? new Calendar(by, ey, 1) : cal; //不用每次都初始化 2006-12-03 修正 
cal.DateMode =pickMode["second"]; //複位 
if(strFormat.indexOf('s')< 0) {cal.DateMode =pickMode["minute"];}//精度爲分 
if(strFormat.indexOf('m')< 0) {cal.DateMode =pickMode["hour"];}//精度爲時 
if(strFormat.indexOf('h')< 0) {cal.DateMode =pickMode["day"];}//精度爲日 
if(strFormat.indexOf('d')< 0) {cal.DateMode =pickMode["month"];}//精度爲月 
if(strFormat.indexOf('M')< 0) {cal.DateMode =pickMode["year"];}//精度爲年 
if(strFormat.indexOf('y')< 0) {cal.DateMode =pickMode["second"];}//默認精度爲秒 
...
2006-06-25 由 寒羽楓 修改爲根據用戶指定的 style 來確定； 
*/ 
...
this.colors = { 
"cur_word" : "#FFFFFF", //當日日期文字顔色 
"cur_bg" : "#00FF00", //當日日期單元格背影色 
"sel_bg" : "#FFCCCC", //已被選擇的日期單元格背影色 2006-12-03 寒羽楓添加 
"sun_word" : "#FF0000", //星期天文字顔色 
"sat_word" : "#0000FF", //星期六文字顔色 
"td_word_light" : "#333333", //單元格文字顔色 
"td_word_dark" : "#CCCCCC", //單元格文字暗色 
"td_bg_out" : "#EFEFEF", //單元格背影色 
"td_bg_over" : "#FFCC00", //單元格背影色 
"tr_word" : "#FFFFFF", //日曆頭文字顔色 
"tr_bg" : "#666666", //日曆頭背影色 
"input_border" : "#CCCCCC", //input控件的邊框顔色 
"input_bg" : "#EFEFEF" //input控件的背影色 
} 
/* //2008-01-29 放到了 show ，因爲要做 pickMode 判斷 
this.draw(); 
this.bindYear(); 
this.bindMonth(); 
*/ 
...
mvAry[mvAry.length] = ' <div align="center" nowrap="nowrap" style="padding:4px 4px 4px 4px;background-color:' + calendar.colors["input_bg"] + ';">'; 
...
mvAry[mvAry.length] = '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:70px;height:20px;font-size:12px;cursor:pointer"/>'; 
...
tds[i].style.cursor ="pointer"; //2007-08-06 由寒羽楓添加，鼠標變成手指狀 
...
//擴展 document.getElementById(id) 多浏覽器兼容性 from meizz tree source 
...
var now = new Date(); 
this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle) ;//2008-01-29 寒羽楓添加 → 若爲空則根據dateFormatStyle初始化日期 
...
//由寒羽楓 2006-06-25 修改 → 把 visibility 變爲 display，並添加失去焦點的事件 //this.setDisplayStyle("select", "hidden"); 
...
this.dateControl.changeEvent = this.dateControl.onchange;//將 onchange 轉成其它函數，以免觸發驗證事件 
