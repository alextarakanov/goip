...
/*tels 6元素的數組*/
		...
		if($_POST[submit2]){
			$crontime=checktime();
			if($_GET[id])
				$db->query("update message set receiverid='$Ida', receiverid1='$Idb', receiverid2='$Idc', msg='$_POST[Memo]', userid=$_SESSION[goip_userid],total='$total',crontime=$crontime where id=$_GET[id]");
			else 
				$db->query("INSERT INTO message (receiverid,receiverid1,receiverid2,msg,userid,crontime,groupid,total) VALUES ('$Ida','$Idb','$Idc','$_POST[Memo]',$_SESSION[goip_userid],$crontime,'$_POST[groupid]','$total')");	
		...
		if($_POST[submit2]){
			...
			if($_GET[id])
				...
				$db->query("INSERT INTO message (type,groupid,groupid1,groupid2,msg,userid,crontime,total) VALUES (1,'$Ida','$Idb','$Idc','$_POST[Memo]',$_SESSION[goip_userid],$crontime,'$total')");	
			do_cron($db,$crontime,$goipcronport);
			exit;
		}
		if(!$Ida) $Ida=0;
...
elseif(isset($_POST['alltype'])) {//向全部人發送
	...
	if($_POST[submit2]){
		...
		if($_GET[id])
			...
			$db->query("INSERT INTO message (type,msg,userid,crontime,recv,recv1,recv2,total) VALUES (2, '$_POST[Memo]', $_SESSION[goip_userid], $crontime,$jr,$jr1,$jr2, '$total')");	
		do_cron($db,$crontime,$goipcronport);
		exit;
	}
...
*/
		...
		foreach($tels as $provname => $provtels){
			foreach($provtels as $send){
				$db->query("INSERT INTO sends (messageid,userid,telnum,provider,recvid,recvlev,msg,total) VALUES ($sendid,$userid,'".$send[tel]."','$provname',".$send[id].",".$send[lev].",'".$send[msg]."','".$send[total]."')");
				$i++;
			}	
		}
